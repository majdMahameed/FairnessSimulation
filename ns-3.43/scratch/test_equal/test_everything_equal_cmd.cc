...
NS_LOG_COMPONENT_DEFINE("TestEverythingEqual"); // define logging component so NS_LOG_* macros work
...
int main(int argc, char *argv[]) {
    uint32_t numFlows = 1; // default value
    std::string dataSendRate = "1Mbps"; // default data rate
    std::string rttDelays = "1ms"; // default: all flows get 10ms
    std::string bottleneckLinkDataRate = "1Mbps"; // default bottleneck link data rate
    std::string bottleneckLinkDelay = "1ms"; // default bottleneck link delay
    std::string simulationTime = "200.0"; // default simulation time
    ...
    for (uint32_t i = 0; i < numFlows; ++i) {
        PointToPointHelper accessLink;
        accessLink.SetDeviceAttribute("DataRate", StringValue(dataSendRate));
        accessLink.SetChannelAttribute("Delay", StringValue("1ms")); // Fixed 10ms delay for all right links
    ...
    if (!useRed) {
        bottleneckLink.SetQueue("ns3::DropTailQueue", "MaxSize", StringValue("80p")); // DropTail for all except DCTCP
    }
    ...
    if (useRed) {
        ...
        Config::SetDefault("ns3::RedQueueDisc::MinTh", DoubleValue(50)); // packets
        Config::SetDefault("ns3::RedQueueDisc::MaxTh", DoubleValue(50)); // packets
        Config::SetDefault("ns3::RedQueueDisc::MaxSize", QueueSizeValue(QueueSize("60p")));
    ...
    std::string cwndFile = "scratch/test_equal/results/cwnd_trace.csv";
    ...
    for (uint32_t i = 0; i < numFlows; ++i) {
        ...
        double startOffset = jitterDist(jitterGen);
    ...
    std::string summaryFile = "scratch/test_equal/results/results_summary.csv";
