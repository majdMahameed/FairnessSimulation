...
NS_LOG_COMPONENT_DEFINE("TestStartDifference"); // define logging component so NS_LOG_* macros work
...
int main(int argc, char *argv[]) {
    uint32_t numFlows = 2; // default value
    std::string dataSendRate = "2Mbps"; // default data rate
    std::string rttDelays = "10ms"; // default: all flows get 10ms
    std::string bottleneckLinkDataRate = "2Mbps"; // default bottleneck link data rate
    std::string bottleneckLinkDelay = "10ms"; // default bottleneck link delay
    ...
    if (!useRed) {
        bottleneckLink.SetQueue("ns3::DropTailQueue", "MaxSize", StringValue("120p")); // DropTail for all except DCTCP
    }
    ...
    if (useRed) {
        ...
        Config::SetDefault("ns3::RedQueueDisc::MinTh", DoubleValue(80)); // packets
        Config::SetDefault("ns3::RedQueueDisc::MaxTh", DoubleValue(240)); // packets
        Config::SetDefault("ns3::RedQueueDisc::MaxSize", QueueSizeValue(QueueSize("320p")));
    ...
    std::string cwndFile = "scratch/test_start/results/cwnd_trace.csv";
    ...
    lastRtt.assign(numFlows + 1, 0.0);
        ...
        if (numFlows >= 2) {
            flowStartOffsets[1] = flowStartOffsets[0] + 15.0; // Second flow: 15s after first (including jitter)
        }
    ...
    std::string summaryFile = "scratch/test_start/results/results_summary.csv";
