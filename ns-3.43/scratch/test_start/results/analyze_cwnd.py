...
df = pd.read_csv('scratch/test_start/results/cwnd_trace.csv')

# Try to extract RTT delays from the last simulation command in run_and_analyze.sh
rtt_delay_list = []
rtt_pattern = re.compile(r'--RttDelays=([^\s]+)')
run_script_path = os.path.join(os.path.dirname(__file__), '../../run_start.sh')
...
for protocol in df['Protocol'].unique():
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'scratch/test_start/results/cwnd_{protocol}.png')
    plt.close()